...
# Updated the secret_table creation query to remove the UNIQUE constraint on the udid column
sudo mysql -u root -e "USE $mysql_db; CREATE TABLE secret_table (id INT AUTO_INCREMENT PRIMARY KEY, udid VARCHAR(255) NOT NULL, secret VARCHAR(255) NOT NULL, computer_id INT NOT NULL, expiration INT NOT NULL, is_active BOOLEAN DEFAULT TRUE);"
